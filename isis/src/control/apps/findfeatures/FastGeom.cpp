...
/* Constructor */
FastGeom::FastGeom(NaifContextPtr naif) : m_naif(naif), m_fastpts(25), m_tolerance(1.0), m_geomtype("camera"),
                       m_maxarea(3.0), m_parameters() {
  validate(m_geomtype);
}

/* Construct with parameters */
FastGeom::FastGeom(NaifContextPtr naif, const PvlFlatMap &parameters) : m_naif(naif), m_fastpts(25),
                                                   m_tolerance(1.0),
                                                   m_geomtype("camera"),
                                                   m_maxarea(3.0),
...
ImageTransform *FastGeom::compute(MatchImage &query, MatchImage &train)  {
  ...
  cv::Mat t_to_q = train.source().getGeometryMapping(m_naif,
                                                     query.source(),
                                                     m_fastpts,
                                                     m_tolerance);
