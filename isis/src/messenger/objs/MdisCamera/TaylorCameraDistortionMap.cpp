...
namespace Isis {
   ...
   */
  void TaylorCameraDistortionMap::SetDistortion(NaifContextPtr naif, const int naifIkCode) {
    QString odtxkey = "INS" + toString(naifIkCode) + "_OD_T_X";
    QString odtykey = "INS" + toString(naifIkCode) + "_OD_T_Y";
    for(int i = 0; i < 10; ++i) {
      p_odtx.push_back(p_camera->getDouble(naif, odtxkey, i));
      p_odty.push_back(p_camera->getDouble(naif, odtykey, i));
    }
  }
