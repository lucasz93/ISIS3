...
namespace Isis {
  ...
  void Spice::init(Pvl &lab, bool noTables, json isd) {
    ...
    if (m_usingNaif) {
      try {
        ...
        if (isd == NULL){
          ...
          isd = ale::load(naif, lab.fileName().toStdString(), props.dump(), "ale");
        }
    ...
    *m_spkBodyCode = m_target->naifBodyCode();
    ...
    if (m_usingAle) {
      m_sunPosition->LoadCache(isd["sun_position"], naif);
    ...
    if (m_usingAle) {
      m_instrumentPosition->LoadCache(isd["instrument_position"], naif);
